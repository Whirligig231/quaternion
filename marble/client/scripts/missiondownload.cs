...
function changeLevel(%file) {
	...
	while (MissionCleanup.getCount() > 0) {
		error(MissionCleanup.getObject(0));
		MissionCleanup.getObject(0).delete();
	}
	error("MissionCleanup is" SPC MissionCleanup.getId());
	MissionCleanup.delete();
    $InstantGroup = ServerGroup;
	
	// Reset events
	exec("marble/server/scripts/reset.cs");
