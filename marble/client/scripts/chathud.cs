...
function playMessageSound(%message, %voice, %pitch)
{
   ...
   if (%voice !$= "") {
      %wavFile = "~/data/Shared/sound/voice/" @ %voice @ "/" @ %wav;
   }
   else {
      %wavFile = "~/data/Shared/sound/" @ %wav;
   }
   ...
   %wavLengthMS = fmodGetWaveLen(%wavFile) * %pitch;
   if (%wavLengthMS == 0) {
      ...
      if ($ClientChatHandle[%sender] != 0) {
         fmodStop($ClientChatHandle[%sender]);
      }
      $ClientChatHandle[%sender] = fmodCreateSource(AudioMessage, %wavFile);
      if (%pitch != 1.0) {
         fmodSourcef($ClientChatHandle[%sender], "AL_PITCH", %pitch);
      }
      fmodPlay($ClientChatHandle[%sender]);
   }
   else {
...
function addChatLine(%message)
{
   if (getWordCount(%message)) {
      %text = "<just:center><font:Exo:32>" @ %message;
