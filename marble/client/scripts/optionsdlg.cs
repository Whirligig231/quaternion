...
function OptionsDlg::onWake(%this)
{
   if ($platform $= "x86UNIX")
      ...
      OptGfxText.setText("<font:Exo:32><just:right>Screen Resolution:\n\nScreen Style:\n\nColor Depth:\n\nStencil Shadows:\n\n");
   else
      OptGfxText.setText("<font:Exo:32><just:right>Screen Resolution:\n\nScreen Style:\n\nColor Depth:\n\n");
...
{
   OptRemapText.setText( "<just:center><font:Exo:24>Press a new key or button for \"" @ %name @ "\"" );
...
function OptAudioUpdateMasterVolume(%volume)
{
   if (%volume == $pref::Audio::masterVolume)
      return;
   fmodSetChannelVolume(1, %volume);
   $pref::Audio::masterVolume = %volume;
   if (($AudioTestHandle $= "" || !fmodIsPlaying($AudioTestHandle)))
   {
      $AudioTestHandle = fmodPlay(TestingSfx);
   }
}
...
function OptAudioUpdateChannelVolume(%channel)
{
   ...
   if (($AudioTestHandle == 0 || !fmodIsPlaying($AudioTestHandle)) && %channel == 1)
   {
      $AudioTestHandle = fmodPlay(TestingSfx);
   }
}
