...
function clientCmdGameEnd()
{
   ...
   if($highScoreIndex !$= "")
   {
      if($highScoreIndex == 0)
         ...
         %msgIn = " 3rd";
      EnterNameText.setText("<just:center><font:Quaternion:50>Congratulations!\n<font:Exo:32>You got the" @ %msgIn @ " best time!");
...
function reformatGameEndText()
{
   ...
   %text = 
      ...
      "<font:Quaternion:50><color:ffff00>Final Time:\t<color:fff090>" @
      formatTime($Game::ScoreTime) @ "<color:000000><font:Quaternion:32>\n<just:center>";
   ...
   if (MissionInfo.time)
      %text = %text @
         "<color:000000>" @
         "<font:Exo:32>Qualify Time:\t" @
         ($Game::Qualified? "<shadowcolor:ffffff7f><color:000000>": "<shadowcolor:0000007f><color:ff0000>") @
         formatTime(MissionInfo.time) @ "\n";
   else
      %text = %text @ "<color:000000><font:Exo:32>Qualify Time:\t\t99:59.99\n";

   if(MissionInfo.goldTime)
   {
      %text = %text @ "<shadowcolor:ffffff7f><color:000000><font:Exo:32>Gold Time:\t<color:FFFF00><shadowcolor:0000007f>" @
         formatTime(MissionInfo.goldTime) @ "\n";
   }

   %text = %text @
      "<shadowcolor:ffffff7f><color:000000>" @
      "<font:Exo:32>Elapsed Time:\t" @ formatTime($Game::ElapsedTime)  @ "\n" @
      "<font:Exo:32>Bonus Time:\t" @ formatTime($Game::BonusTime) @ "\n";
   %text = %text @ "<font:Arial:14>\n<font:Exo:32><color:000000>Best Times:\n";
   for(%i = 0; %i < 3; %i++)
   {
      %time = getField($hs[%i], 0);
      %name = getField($hs[%i], 1);
      %text = %text 
         @ "<shadowcolor:ffffff7f><color:000000><font:Exo:32>" @ %i+1 @ ". " @ %name @ "\t" 
