...
{
   cancelAll(%this);
   if (%this.getDataBlock().noRespawn)
      return;
   %this.hide(false);
...
{
	%halfExtent = VectorScale(VectorSub(getWords(%obj.getWorldBox(), 3), getWords(%obj.getWorldBox(), 0, 2)), 0.5);
	%obj.btShape = btCreateShapeBox(%halfExtent);
	%pos = %obj.getWorldBoxCenter();
	%obj.btBody = btCreateBody(%obj.btShape, 0, %pos SPC "1 0 0 0");
	btSetSolid(%obj.btBody, false);
	$btGlobalBodyIndex[%obj.btBody] = %obj;
}

function ItemData::onFrame(%this, %obj)
{
	...
	if (%iscol && !%prevcol)
		%obj.getDatablock().onPickup(%obj, p(), 1);
