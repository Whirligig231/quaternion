...
function State::ready()
{
   ...
   $Game::StateSchedule = schedule( 0, 0, "setGameState", "go");
}
...
function State::set()
{
   ...
   $Game::StateSchedule = schedule( 10, 0, "setGameState", "Go");
}
...
function GameConnection::onClientEnterGame(%this)
{
   ...
   MissionPersistent.add( %this.camera );
...
function GameConnection::respawnPlayer(%this)
{
   ...
   %this.player.setMode(Start);
   %this.player.setPositionB(%this.getCheckpointPos(), 0.45);
...
function GameConnection::createPlayer(%this, %spawnPoint)
{
   ...
   MissionPersistent.add(%player);

   // Player setup...
   %player.setPositionB(%spawnPoint, 0.45);
...
{
   if (!isObject($Game::SpawnPad))
		findSpawnPad(MissionGroup);
   return vectorAdd($Game::SpawnPad.getTransform(),"0 0 0.5") SPC
                    getWords($Game::SpawnPad.getTransform(), 3);
}
...
function findSpawnPad(%group)
{
   for (%i = 0; %i < %group.getCount(); %i++)
   {
      ...
      if (%type $= "SimGroup")
         ...
         if (%type $= "StaticShape" &&
               %object.getDatablock().getName() $= "StartPad" && %object.index == $Game::SpawnIndex)
            $Game::SpawnPad = %object;
   }
}
