...
function WorldEditor::playSelection(%this)
{
	if (%this.getSelectionSize() > 0) {
		for (%i = 0; %i < %this.getSelectionSize(); %i++) {
			%this.getSelectedObject(%i).setActive(1);
		}
	}
	else {
		for (%i = 0; %i < ServerConnection.getCount(); %i++) {
			ServerConnection.getObject(%i).getServer().setActive(1);
		}
	}
}

function WorldEditor::stopSelection(%this)
{
	if (%this.getSelectionSize() > 0) {
		for (%i = 0; %i < %this.getSelectionSize(); %i++) {
			%this.getSelectedObject(%i).setActive(0);
		}
	}
	else {
		for (%i = 0; %i < ServerConnection.getCount(); %i++) {
			ServerConnection.getObject(%i).getServer().setActive(0);
		}
	}
}

function WorldEditor::setPosSelection(%this)
{
	if (%this.getSelectionSize() > 0) {
		for (%i = 0; %i < %this.getSelectionSize(); %i++) {
			%this.getSelectedObject(%i).setInitialPosition();
		}
	}
	else {
		for (%i = 0; %i < ServerConnection.getCount(); %i++) {
			ServerConnection.getObject(%i).getServer().setInitialPosition();
		}
	}
}
