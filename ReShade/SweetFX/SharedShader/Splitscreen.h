...
float4 SplitscreenPass( float4 colorInput, float2 tex )
{
  // -- Vertical 50/50 split --
	  ...
	  return (tex.x < 0.5) ? myTex2D(RFX_originalColor, tex) : colorInput;
  #endif
  ...
  // -- Vertical 25/50/25 split --
    ...
    return distance ? myTex2D(RFX_originalColor, tex) : colorInput;
	#endif
  ...
  // -- Vertical 50/50 angled split --
    ...
    return distance ? colorInput : myTex2D(RFX_originalColor, tex);
	#endif
  
  // -- Horizontal 50/50 split --
  #if splitscreen_mode == 4
	  return (tex.y < 0.5) ? myTex2D(RFX_originalColor, tex) : colorInput;
  #endif
