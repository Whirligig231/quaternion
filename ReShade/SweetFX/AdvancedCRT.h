...
// SFX_aspect ratio
#define SFX_aspect float2(1.0, 0.75)
...
float3 maxscale()
{
	...
	float2 a = float2(0.5, 0.5) * SFX_aspect;
	float2 lo = float2(fwtrans(float2(-a.x, c.y)).x, fwtrans(float2(c.x,-a.y)).y) / SFX_aspect;
	float2 hi = float2(fwtrans(float2(+a.x, c.y)).x, fwtrans(float2(c.x, +a.y)).y) / SFX_aspect;
	return float3((hi + lo) * SFX_aspect * 0.5, max(hi.x - lo.x, hi.y - lo.y));
}

float2 transform(float2 coord, float2 textureSize, float2 inputSize)
{
	coord *= textureSize / inputSize;
	coord = (coord - 0.5) * SFX_aspect * stretch.z + stretch.xy;
	return (bkwtrans(coord) / float2(CRTOverScan, CRTOverScan) / SFX_aspect + 0.5) * inputSize / textureSize;
}
...
float corner(float2 coord, float2 textureSize, float2 inputSize)
{
	...
	coord = min(coord, 1.0 - coord) * SFX_aspect;
